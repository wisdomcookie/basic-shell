---
title: CS406 Individual Project 1 "Simple Shell" (Due Feburary 25th by 11:59pm)
geometry: margin=1in
output: pdf_document
fontsize: 10pt
---
...
A `Makefile` is provided that will perform the following:
  ...
  * **test** will run a set of unit tests against your program if you are just doing a normal make file compile.
...
This project can be developed using any IDE you wish on your personal machines.  But should use the makefile to build the program and test it on the server.  This can be done from the command-line.  Please be aware, just because it runs perfectly in your environment does not mean it will run the same on the server where it will be graded.

To submit your project, you need to provide a zipped or tar zipped version to a
submission on the Moodle.  The tar command can create this, and an example is provided here.

Also be sure to both provide a document explaining your implementation process and your name on all code identifying it as your solution.

```
   tar cvfz destination-archive.tgz source-directory/
```
...
Your basic shell, called `lsh` (short for Lafayette Shell), is
basically an interactive loop: it repeatedly prints a prompt `lsh> ` (note
